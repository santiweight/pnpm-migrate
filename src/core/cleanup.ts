...
function buildCleanupPrompt(worktree: MigrationWorktree, deterministicLogPath: string): string {
  ...
  return [
    ...
    "",
    "TS2742 guidance:",
    "- If TypeScript fails with TS2742 after pnpm migration, prefer explicit portable type annotations on exported declarations.",
    "- Use public types from direct dependencies; do not fix TS2742 by adding hoist patterns or random @types packages unless the error is specifically a missing declared type dependency.",
    "- Example: change `import express from \"express\"; export function createApp() { return express(); }` to `import express, { type Express } from \"express\"; export function createApp(): Express { return express(); }`.",
    "- If several TS2742 errors appear, handle them as a sequence and rerun the relevant TypeScript build after each minimal batch.",
    "",
    "Constraints:",
