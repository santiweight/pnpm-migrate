...
function scriptsOf(pkg: Record<string, unknown>): Record<string, unknown> {
  return typeof pkg.scripts === "object" && pkg.scripts !== null && !Array.isArray(pkg.scripts)
    ? pkg.scripts as Record<string, unknown>
    : {};
}

function workspacesOf(pkg: Record<string, unknown>): unknown[] {
  ...
  if (
    ...
    return (pkg.workspaces as { packages: unknown[] }).packages;
  }
  return [];
}
...
export function validateMigration(inputRoot: string): ValidationResult {
  ...
  if (!exists(root, "package.json")) {
    ...
    if (!/^pnpm@/.test(String(pkg.packageManager || ""))) {
      errors.push("package.json packageManager must be set to pnpm@<version>");
    }

    if (workspacesOf(pkg).length > 0 && !exists(root, "pnpm-workspace.yaml")) {
  ...
  for (const file of walk(root).filter((entry) => path.basename(entry) === "package.json")) {
    ...
    for (const [name, script] of Object.entries(scriptsOf(pkg))) {
      if (typeof script !== "string") continue;
      if ((hardNpmPattern.test(script) || npmScriptShorthandPattern.test(script)) && releaseScriptNamePattern.test(name)) {
        warnings.push(`${rel} release script "${name}" contains npm/npx command requiring maintainer review: ${script}`);
      } else if (hardNpmPattern.test(script) || npmScriptShorthandPattern.test(script)) {
        errors.push(`${rel} script "${name}" still contains npm/npx command: ${script}`);
      }
      if (badPnpmPattern.test(script)) {
        errors.push(`${rel} script "${name}" contains suspicious pnpm rewrite: ${script}`);
      }
      if (removedLockfilePattern.test(script)) {
        errors.push(`${rel} script "${name}" still references removed npm lockfile: ${script}`);
      }
      if (unsupportedPnpmLockfileLintPattern.test(script)) {
  ...
  const proseFiles = walk(root).filter((file) => relative(root, file).endsWith(".md"));
...
export function formatValidationResult(result: ValidationResult): string {
  ...
  if (result.warnings.length > 0) {
    ...
    result.warnings.slice(0, 120).forEach((warning) => lines.push(`  - ${warning}`));
    if (result.warnings.length > 120) {
      lines.push(`  - ... ${result.warnings.length - 120} additional warnings omitted`);
    }
  }
